...
public partial class BusinessPage : ContentPage
{
    ...
    // Note: it is unnecessary to specify WaitingList = 8 as it is calculated when displaying the Businesses.
    ...
    private async Task<List<BusinessModel>> GetBusinessesInQueue(int userId)
    {
        ...
        // Get user position for each business
        foreach (var business in businessList)
        {
            // Use the GetUserQueuePosition method to get the user's position for the current business
            var userPosition = await GetUserQueuePosition(userId, business.Id);
            business.WaitingList = userPosition;
        }
    ...
    private async Task<List<BusinessModel>> GetBusinessesNotInQueue(int userId)
    {
        ...
        // Get waiting list for each business
        foreach (var business in businessesNotInQueue)
        {
            // Use the GetUserQueuePosition method to get the user's position for the current business
            var waitingList = await GetBusinessQueueItems(business.Id);
            business.WaitingList = waitingList.Count;
        }

        return businessesNotInQueue;
    }

    private async Task<List<QueueItemModel>> GetBusinessQueueItems(int businessId)
    {
    // Gives the ordered queue for a specific business
        var response = await firebaseClient.Child("QueueItems")
            .OrderBy(nameof(QueueItemModel.JoinTimestamp)) //TODO: Check that the ordering is chronological
            .OnceAsync<QueueItemModel>();

        var businessQueueItems = response
            .Where(item => item.Object.BusinessId == businessId)
            .Select(item => item.Object)
            .ToList();

        return businessQueueItems;
    }

    private async Task<int> GetUserQueuePosition(int userId, int businessId)
    {
        var businessQueueItems = await GetBusinessQueueItems(businessId);

        var userPosition = businessQueueItems
            .Select((item, index) => new { Item = item, Index = index })
            .FirstOrDefault(entry => entry.Item.UserId == userId)?.Index;

        // If userPosition is not null, return the position + 1 (1-indexed), otherwise return 0
        return userPosition.HasValue ? userPosition.Value + 1 : 0;
    }
